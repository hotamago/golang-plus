 ...
 */
function findGoModDir(filePath) {
 ...
 */
function resolveImportPath(importPath, documentPath) {
 ...
 */
function findSymbolInDir(dirPath, symbolName) {
    try {
...
class GoplusDefinitionProvider {
            ...
            if (position.character > dotCharPos) {
                ...
                if (importPath) {
                    ...
                    if (resolved) {
                        ...
                        if (result) {
                            return new vscode.Location(vscode.Uri.file(result.file), new vscode.Position(result.line, result.col));
                        }
                    }
                }
            }
                    ...
                    if (resolved && fs.existsSync(resolved)) {
                        try {
                            ...
                            for (const entry of entries) {
                                const ext = path.extname(entry);
                                if ((ext === '.gp' || ext === '.go') && !entry.endsWith('_test.go')) {
                                    return new vscode.Location(vscode.Uri.file(path.join(resolved, entry)), new vscode.Position(0, 0));
                                }
                            }
                        }
                        catch {
     ...
     */
    findImportForPackage(document, pkgName) {
